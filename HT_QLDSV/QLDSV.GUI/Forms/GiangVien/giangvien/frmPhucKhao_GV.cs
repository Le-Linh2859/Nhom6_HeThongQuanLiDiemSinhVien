...
{
    public partial class frmPhucKhao_GV : Form, IShellChildForm
    {
        ...
        private void BuildSidebarMenu()
        {
            ...
            for (int i = 0; i < menuItems.Length; i++)
            {
                ...
                btn.Click += (s, ev) =>
                {
                    ...
                    {
                        targetForm = new QLDSV.GUI.Forms.GiangVien.frmLophocphan_GV();
                    }
                    else if (itemText == "Đăng ký lớp")
                    {
                        targetForm = new frmDangKyHocPhan();
                    }
                    else if (itemText == "Nhập điểm")
                    {
        ...
        private void frmPhucKhao_GV_Load(object sender, EventArgs e)
        {
            try
            {
                FunctionQa.ketnoi();

                // Get lecturer code associated with this account
            ...
            {
                MessageBox.Show("Lỗi khởi tạo form giảng viên: " + ex.Message,
        ...
        private void TaiDuLieu()
        {
            try
            {
                ...
                // Giảng viên xem đơn của sinh viên trong lớp mình phụ trách
                // Cột: Mã PK, Họ Tên SV, Mã SV, Lớp HP, Ngày YC, Trạng Thái, Lý Do
                string sql = "SELECT pk.MaPhucKhao AS [Mã PK], " +
                             ...
                             $"WHERE pk.MaLHP IN (SELECT MaLHP FROM LopHocPhan WHERE MaGV = '{maGVHienTai}')";
                ...
                if (dataGridView.Columns.Contains("Họ Tên SV"))    dataGridView.Columns["Họ Tên SV"].Width = 140;
                if (dataGridView.Columns.Contains("Mã SV"))         dataGridView.Columns["Mã SV"].Width = 90;
                if (dataGridView.Columns.Contains("Lớp Học Phần"))  dataGridView.Columns["Lớp Học Phần"].Width = 160;
            ...
            {
                MessageBox.Show("Lỗi tải dữ liệu phúc khảo giảng viên: " + ex.Message, "Lỗi",
        ...
        private void ShowViewMode(string maPK, string maSV, string maLHP, string ngayYCStr, string trangThai, string lyDo)
        {
            ...
            // Inputs disabled for lecturer view detail
